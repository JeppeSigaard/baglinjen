...
add_action( 'init', 'smamo_rest_api_featured_images_init', 12 );

function smamo_rest_api_featured_images_init() {
		...
		if ( $show_in_rest && $supports_thumbnail ) {
			...
			if ( function_exists( 'register_rest_field' ) ) {
				register_rest_field( $post_type_name,
					'featured_image',
					array(
						'get_callback' => 'smamo_rest_api_featured_images_get_field',
						'schema'       => null,
					)
				);
...
function smamo_rest_api_featured_images_get_field( $object, $field_name, $request ) {
